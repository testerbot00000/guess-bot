...
namespace guess_bot
{
    ...
    {
        //I know this is bad form but whatever
        private static int Number = -1;
        private static int Tries = 0;
        public static bool GameRunning = false;
        public static ulong ChannelID = 0;

        private static string LastWinnerMention = "";
        private static int LastWinnerCount = 0;
        private static int LastWinnerTries = 0;
        private static bool HadLastWinner = false;
        ...
        public async Task LastWinner()
        {
            if (HadLastWinner)
            {
                ...
                emb.AddField("Winner", LastWinnerMention);
                emb.AddField("Tries", LastWinnerTries);
                emb.AddField("Number", LastWinnerCount);
                
                await ReplyAsync("", false, emb.Build());
            }
            else
            {
                await ReplyAsync("There hasn't been a winner");
            }
        }
        ...
        public static async Task GuessNumber(int number, ISocketMessageChannel channel, IUser user, IGuildChannel guildChannel, CommandContext context)
        {
            if (GameRunning)
            {
                if (number == Number)
                {
                    ...
                    LastWinnerMention = user.Mention;
                    LastWinnerCount = Number;
                    LastWinnerTries = Tries + 1;
                    HadLastWinner = true;
